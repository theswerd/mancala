...
#[derive(Debug, Clone)]
pub struct BankBird4<const S: usize>(pub usize); // depth
impl<const S: usize> Algorithm<S> for BankBird4<S> {
    fn name(&self) -> String { format!("Bank Bird v4 (d:{})", self.0) }
...
fn calculate_score_v4<const S: usize>(board: &MancalaBoard<S>) -> i32 {
    if board.game_over() {
        return match board.winner() {
            Winner::Side(Side::Left) => 500,
            Winner::Side(Side::Right) => -500,
            Winner::Tie => 0,
        }
    }
